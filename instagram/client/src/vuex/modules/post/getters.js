...
export const getPostCount = state => {
  if (state.posts.length > 0) {
    return state.posts[0].totalElements;
  }
...
export const getCurrentPageFollowingPosts = state => {
  if (state.followingPosts.length > 0) {
    return state.followingPosts[state.followingPosts.length - 1]
      .currentPageNumber;
  }
  return 0;
};
...
export const getLastCommentFollowingPosts = state => postId => {
  if (state.followingPosts.length > 0) {
    const comments = state.followingPosts.filter(post => post.id === postId)[0]
      .comments;
